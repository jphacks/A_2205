...
def crawl_tweets(username: str):
    ...
    tweet_info = api.get_liked_tweets(id=user_id, expansions="author_id")
    ...
    df_tweets_new = pd.DataFrame.from_records(
        [{"id": tweet.id, "text": tweet.text, "author_name": author.username, "annotated": False} for author, tweet in zip(author_info, liked_tweets) if tweet.id not in tweet_set]
    )
